...
@section('content')
    ...
    <div class="space-y-6">
        ...
        <x-admin.module>
            <h2 class="text-base font-semibold text-slate-900">Create Category</h2>
            <form method="POST" action="{{ route('admin.categories.store') }}" class="mt-4 grid gap-4 sm:grid-cols-2">
                ...
                <div class="sm:col-span-2">
                    <button type="submit" class="rounded-xl bg-emerald-700 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-600">Create Category</button>
                </div>
            </form>
        </x-admin.module>
        ...
        <x-admin.module variant="table">
            <x-slot:header>
                <h2 class="text-base font-semibold text-slate-900">Category List</h2>
            </x-slot:header>
            ...
            <form method="GET" class="grid gap-3 border-b border-slate-200 bg-white p-4 sm:grid-cols-4 sm:p-6">
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="q" value="{{ $query }}" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm" placeholder="Search category name or icon...">
                </div>
                <div>
                    <label class="mb-1 block text-xs font-semibold text-slate-700">Icon</label>
                ...
                <div class="flex items-end gap-2">
                    ...
                    <button type="submit" class="rounded-lg bg-slate-900 px-3 py-2 text-sm font-semibold text-white hover:bg-slate-800">Apply</button>
                </div>
                <div>
                ...
                <table class="min-w-full">
                    <thead>
                        <tr class="bg-emerald-700 text-white">
                            ...
                            <th class="px-3 py-2 text-left text-xs font-semibold">Order</th>
                            <th class="px-3 py-2 text-left text-xs font-semibold">Icon</th>
                            <th class="px-3 py-2 text-left text-xs font-semibold">Products</th>
                            ...
                            <tr class="border-t border-slate-200 bg-white">
                                ...
                                <td class="px-3 py-2">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="{{ route('admin.categories.edit', $category->id) }}" class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-slate-300 text-slate-700 hover:bg-slate-100" aria-label="Edit category {{ $category->name }}">
            ...
            <x-slot:footer>
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <p class="text-sm text-slate-600">
                        Showing <span class="font-semibold text-slate-900">{{ $categories->count() }}</span>
                        of <span class="font-semibold text-slate-900">{{ $filteredCount }}</span> filtered categories
                        ({{ $totalCount }} total)
                    </p>
